...
`--moduleResolution`                           | Default: `"Classic"`<br/> <br/> Type: `ModuleResolutionKind` | Determine how modules get resolved. Either 'node' for Node.js/io.js style resolution, or 'classic' (default).
`--newLine`                                    | Default:<br/>*(platform specific)*<br/> <br/> Type: `string`     | Use the specified end of line sequence to be used when emitting files: 'CRLF' (dos) or 'LF' (unix)."
...
`--project`<br/>`-p`                           | Default: `"."`<br/> <br/> Type: `string`                     | Compile the project in the given directory. The directory needs to contain a `tsconfig.json` file to direct compilation. See [tsconfig.json](./tsconfig.json.md) documentation for more details.
...
`--rootDir`                                    | Default: `"."`<br/> <br/> Type: `string`                     | Specifies the root directory of input files. Only use to control the output directory structure with `--outDir`.
...
`--suppressExcessPropertyErrors`<sup>[1]</sup> | Default: `false`<br/> <br/> Type: `boolean`                  | Suppress excess property checks for object literals.
