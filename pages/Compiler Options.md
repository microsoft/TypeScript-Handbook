...
Option                                         | Type      | Default                        | Description | Introduced
-----------------------------------------------|-----------|--------------------------------|----------------------------------------------------------------------|---
`--allowJs`                                    | `boolean` | `true`                         | Allow JavaScript files to be compiled. | 1.8
`--allowSyntheticDefaultImports`               | `boolean` | `module === "system"`          | Allow default imports from modules with no default export. This does not affect code emit, just typechecking. | 1.8
`--allowUnreachableCode`                       | `boolean` | `false`                        | Do not report errors on unreachable code. | 1.8
`--allowUnusedLabels`                          | `boolean` | `false`                        | Do not report errors on unused labels. | 1.8
`--baseUrl`                                    | `string`  |                                | Base directory to resolve non-relative module names. See [Module Resolution documentation](./Module Resolution.md#base-url) for more details. | 2.0
`--charset`                                    | `string`  | `"utf8"`                       | The character set of the input files. | 1.0
`--declaration`<br/>`-d`                       | `boolean` | `false`                        | Generates corresponding '.d.ts' file. | |
`--declarationDir`                             | `string`  |                                | Output directory for generated declaration files. | 2.0
...
`--inlineSourceMap`                            | `boolean` | `false`                        | Emit a single file with source maps instead of having a separate file. | 1.5
`--inlineSources`                              | `boolean` | `false`                        | Emit the source alongside the sourcemaps within a single file; requires `--inlineSourceMap` or `--sourceMap` to be set. | 1.5
`--init`                                       |           |                                | Initializes a TypeScript project and creates a `tsconfig.json` file. | 1.6
`--isolatedModules`                            | `boolean` | `false`                        | Unconditionally emit imports for unresolved files. | |
`--jsx`                                        | `string`  | `"Preserve"`                   | Support JSX in '.tsx' files: `'React'` or `'Preserve'`. See [JSX](./JSX.md). | 1.6
...
`--listEmittedFiles`                           | `boolean` | `false`                        | Print names of generated files part of the compilation. | 2.0
...
`--rootDir`                                    | `string`  | *(common root directory is computed from the list of input files)*   | Specifies the root directory of input files. Only use to control the output directory structure with `--outDir`. | 1.5
...
`--skipLibCheck`                               | `boolean` | `false`                        | Don't check a the default library (`lib.d.ts`) file's validity. | 2.0
