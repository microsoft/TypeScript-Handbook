...
`--jsx`                                        |           | `<JsxEmit>`              | `JsxEmit.Preserve` | Support JSX in '.tsx' files: 'React' or 'Preserve'. See [JSX](./JSX.md).
...
`--module`                                     | `-m`      | `<ModuleKind>`           | `(target === ScriptTarget.ES6 ? ModuleKind.ES6 : ModuleKind.CommonJS)` | Specify module code generation: 'commonjs', 'amd', 'system', 'umd', or 'es2015'. Only 'amd' and 'system' can be used in conjunction with `--outFile`. The 'es2015' value may not be used when targeting ES5 or lower.
`--moduleResolution`                           |           | `<ModuleResolutionKind>` | `ModuleResolutionKind.Classic` | Determine how modules get resolved. Either 'node' for Node.js/io.js style resolution, or 'classic' (default).
...
`--target`                                     | `-t`      | `<ScriptTarget>`         | `ScriptTarget.ES5` | Specify ECMAScript target version: 'ES3' (default), 'ES5', or 'ES6'<sup>[1]</sup>
