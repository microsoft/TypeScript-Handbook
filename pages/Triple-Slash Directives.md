...
For declaration file authors who rely on built-in types, e.g. DOM APIs or built-in JS run-time constructors like `Symbol` or `Iterable`, triple-slash-reference lib directives are the recommended approach. Previously these .d.ts files had to add forward/duplicate declarations of such types.
